...
namespace WebApp.Controllers
{
    public class StudentController : Controller
    {
        ...
        public IActionResult CourseResult(int studentId, int courseId)
        {
            StudentBL studentBL = new StudentBL(_context);
            var student = studentBL.GetById(studentId);
            var course = _context.Courses.Find(courseId);
            var enrollment = _context.StudentCourses
                                     .FirstOrDefault(e => e.StudentId == studentId && e.CourseId == courseId);

            if (student == null || course == null || enrollment == null)
            {
                return NotFound();
            }

            var passed = enrollment.Grade >= course.MinDegree;

            var model = new StudentCourseViewModel
            {
                StudentName = student.Name,
                CourseName = course.Name,
                Degree = enrollment.Grade,
                DegreeColor = passed ? "green" : "red"
            };
